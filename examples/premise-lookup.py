...
def parse_args() -> argparse.Namespace:
    ...
    Returns:
    	argparse.Namespace: Parsed arguments with attributes:
    		...
    		- apartment: Apartment or unit number, or None if not provided
    """
...
def pretty_print(data: object) -> None:
    ...
    Parameters:
        data (object): A JSON-serializable value (e.g., dict, list, primitives) to be formatted and printed.
    """
...
async def main() -> None:
    """
    Lookup a National Grid premise using CLI-provided address fields and print the results.
    
    Parses command-line address arguments, performs a premise lookup via the National Grid client, and prints either a "no premises found" message or the number of premises and their JSON payload to stdout. This function performs network I/O and uses an aiohttp session and client context managers for the request lifecycle.
    """
    args = parse_args()
